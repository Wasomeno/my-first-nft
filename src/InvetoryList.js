...
const InventoryList = ({account, stakeBeanz}) => {
    ...
    return(
            <div className="vh-100">
                <h1 id="inventory-title" className="p-2 m-2">Your Beanz</h1>
                {isConnected ? (
                    <div className="d-flex justify-content-center m-2 g-0">
                    {beanzs[0] ? (
                        beanzs.map((beanz, index) => (
                            <div id="inventory-card" key={index} className="card col-2 m-1">
                                <img src={beanz.data.image} className="card-img-top p-2" alt="..."/>
                                    ...
                                    {stakeBeanz ? (
                                        <button id={"stake-button" + index} onClick={() => stakeBeanz((beanz.data.name).slice(6, 7))} className="btn btn-success">Adventure</button>
                                    ) : (
                                        <></>
                                    )}
                            </div>
