...
const MarketplaceItem = ({ account }) => {
  ...
  async function buy() {
    try {
      await contract.buyBeanz(id, signer.getAddress(), {
        value: beanzDetail.beanzPrice[0],
      });
    } catch (error) {
      console.log(error);
    }
  ...
  async function getBeanzDetails() {
    ...
    await contract.beanzOnSale(id).then((response) => {
      beanzPrice = response;
    });
  ...
  return (
    <div className="d-flex p-2 m-2 justify-content-center h-75">
      {isConnected ? (
        isLoading ? (
              ...
              {beanzDetail.beanzPrice.owner.toLowerCase() === account[0] ? (
                ...
                <div className="d-flex flex-wrap align-items-center justify-content-evenly">
                  <div className="col-2 d-flex">
                    ...
                    />
                    <h4 className="m-2">
                      {beanzDetail.beanzPrice[0].toString() / 10 ** 18}
                    </h4>
                  </div>
                  <button
