                 ...
                 <div className="col-6 d-flex d-flex flex-column align-items-center">
                     ...
                     <div className="attributes row w-75">
                         {beanz.attributes.map((attribute,index) => (
                             <span key={index} className="badge rounded-pill bg-primary col p-2 m-1 fw-normal">{attribute.trait_type} : {attribute.value}</span>
                         ))}
                     </div>
                 </div>
                 <div className="col-6 d-flex flex-column justify-content-center">
                     <div className="card">
                         <div className="card-body">
                             ...
                             <input type="text" id="inputPassword5" onChange={(e) => setPrice(e.target.value)} className="form-control" value={price} />
                             {isApproved === MarketplaceContract ? (
                                 <button onClick={() => sell()} className="btn btn-success m-2 w-25">Sell</button>
                             ) : (
                                <button onClick={() => sell()} className="btn btn-primary m-2 w-25">Approve</button>
                             )}
